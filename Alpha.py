...
def helper():
    ...
    print("--------")
    print(
        "\t[" + proc_high + "]: Scan process may take longer times (not predictable)."
    )
    ...
    print(
        "\t"
        + vul_info("c")
        + ": Requires immediate attention as it may lead to compromise or service unavailability."
    )
    print(
        "\t"
        + vul_info("h")
        + "    : May not lead to an immediate compromise, but there are high chances of probability."
    )
    print(
        "\t"
        + vul_info("m")
        + "  : Attacker may correlate multiple vulnerabilities of this type to launch a sophisticated attack."
    )
    print(
        "\t"
        + vul_info("l")
        + "     : Not a serious issue, but it is recommended to attend the finding."
    )
    print(
        "\t"
        + vul_info("i")
        + "    : Not classified as a vulnerability, simply an useful informational alert to be considered.\n"
    )
    ...
    def spinner_task(self):
        try:
            while self.busy:
                ...
                print(
                    bcolors.BG_ERR_TXT + next(self.spinner_generator) + bcolors.ENDC,
                    end=" ",
                )
                sys.stdout.flush()
            ...
            print(
                "\n\t"
                + bcolors.BG_ERR_TXT
                + "Scanner received a series of Ctrl+C hits. Quitting..."
                + bcolors.ENDC
            )
            sys.exit(1)
...
tools_fix = [
    ...
    [
        33,
        "Hackers will be able to read community strings through the service and enumerate quite an information from the target. Also, there are multiple Remote Code Execution and Denial of Service vulnerabilities related to SNMP services.",
    ...
    if target == "--update" or target == "-u" or target == "--u":
        ...
        if oldversion_hash == newversion_hash:
            ...
            print(
                "\t"
                + bcolors.OKBLUE
                + "You already have the latest version of Scanner."
                + bcolors.ENDC
            )
        else:
            clear()
            print(
                "\t"
                + bcolors.OKGREEN
                + "Scanner successfully updated to the latest version."
                + bcolors.ENDC
            )
        spinner.stop()
        sys.exit(1)
        ...
        while sa_avail_tools < len(tools_precheck):
            ...
            try:
                ...
                print(
                    "\t"
                    + bcolors.BG_ERR_TXT
                    + "Scanner was terminated abruptly..."
                    + bcolors.ENDC
                )
                sys.exit(1)
            if "not found" in str(val):
                ...
                for scanner_index, scanner_val in enumerate(tool_names):
                    if scanner_val[2] == tools_precheck[sa_avail_tools][arg1]:
                        ...
                        scanner_val[3] = 0
                        unavail_tools_names.append(tools_precheck[sa_avail_tools][arg1])
        ...
        if unavail_tools == 0:
            print(
                "\t"
                + bcolors.OKGREEN
                + "All Scanning Tools are available. All vulnerability checks will be performed by Scanner."
                + bcolors.ENDC
            )
        else:
            print(
                ...
                + bcolors.ENDC
                + bcolors.WARNING
                + " are unavailable. Scanner can still perform tests by excluding these tools from the tests. Please install these tools to fully utilize the functionality of Scanner."
                + bcolors.ENDC
            )
        print(
            bcolors.BG_ENDL_TXT
        ...
        while tool < len(tool_names):
            ...
            if tool_names[tool][arg4] == 0:
                print(
                    bcolors.WARNING
                    + "...Scanning Tool Unavailable. Auto-Skipping Test..."
                    + bcolors.ENDC
                )
            ...
            if runTest == 1:
                ...
                print(
                    "\t"
                    + bcolors.WARNING
                    + "Test Skipped. Performing Next. Press Ctrl+Z to Quit Scanner."
                    + bcolors.ENDC
                )
                sa_skipped_checks = sa_skipped_checks + 1
        ...
        print(
            bcolors.BG_ENDL_TXT + "[ Preliminary Scan Phase Completed. ]" + bcolors.ENDC
        )
        print("\n")
        ...
        if len(sa_vul_list) == 0:
            print(
                "\t" + bcolors.OKGREEN + "No Vulnerabilities Detected." + bcolors.ENDC
            )
        else:
        ...
        print(
            ...
            + bcolors.BOLD
            + bcolors.OKBLUE
            + display_time(int(sa_total_elapsed))
            + bcolors.ENDC
        )
        print("\n")
        print(
